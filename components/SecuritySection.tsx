...
const SecuritySection = () => {
    const features = [
        {
            icon: <HiLockClosed className="w-5 h-5 text-[var(--brand-text)]" />,
            title: "RBAC & SSO",
            description:
                "Granular roles, SSO/SAML, per-object permissions mapped to ERP users.",
        },
        {
            icon: <HiShieldCheck className="w-5 h-5 text-[var(--brand-text)]" />,
            title: "Audit & logging",
            description:
                "Every action is logged with who/when/what. Exportable trails for auditors.",
        },
        {
            icon: <HiDatabase className="w-5 h-5 text-[var(--brand-text)]" />,
        ...
        <section id="security" className="mx-auto max-w-7xl px-4 py-16 md:py-20">
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-3">
                {features.map((feature, i) => (
                    <div
                        key={i}
                        className="group rounded-2xl p-6 shadow-sm bg-white card transition hover:shadow-md"
                    >
            ...
            <div className="mt-6 text-center text-xs text-[var(--brand-subtext)]">
